...
metapolatortestApp.factory('sliderService', function(){
    return {
        createSlider : function (slider) {
            console.log(slider);
            var sliderEl = angular.element('<input>').attr({id:slider.Elid, type:"text"});
            $(sliderEl).slider();
            return 'something';
        }
    }
...
metapolatortestApp.directive('fontSlider', function(){
    return {
        link: function($scope, element, attrs) {
            var config = $scope.sliders[attrs.fontSlider];
            $(element).after($('<span></span>').attr({id:config.Elid+'Value'}).text(config.value));
            $(element).before($('<span></span>').text(config.name));
            $(element).slider(config).on('slide', function(e){
                $('#'+config.Elid+'Value').text(Number(e.value).toFixed(2));
            });
...
metapolatortestApp.directive('canvasHolder', function(){
   return {
       link: function($scope, element, attrs) {
            //TODO: create canvas creatin directive
       }
   }
});

metapolatortestApp.controller('SliderCtrl', function($scope, sliderService){
    $scope.createSlider = sliderService.createSlider;
    ...
    $scope.createSliders = function (){
        return (this.sliders.id).slider(this.sliders[0]);
    }

     $scope.createCanvas = function(el, x, y) {
        if (!x) {
    ...
    $scope.Instances = function(fontslist, config) {
