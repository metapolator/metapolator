...
Having a preexisting example we all know well makes it easier for us to
think about it.
...
we might be able to use his results to re-create old typefaces. I'm not sure
the kind of font needed to improve OCR-systems is the same as one would
use for typesetting.
...
### The Skeleton
Is the description of our "middle line". We take the number and initial
...
#### Problem:
Being very strict about parameters, **the skeleton points are parameters too**.
Especially the coordinates will be overridable via CSS. We might go so far,
...
* Point incompatibilities: we have no idea how to interpolate between
...
TODO: words about compatibility, what is a compatible outline/skeleton
...
* broker takes data and saves it or rejects to save it
  ...
  if something went wrong, and display an appropriate message.
